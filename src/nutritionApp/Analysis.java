...
public class Analysis {
    ...
    public double func_Q(int xi, int xa) throws ExMathError {
    	if(xi < 0 || xa < 0) {
    ...
    public double food_biased_weight(int cal, int fats, int prot, int carbs, double hyd, double adher) throws ExMathError {
    	if (cal < 0 || fats < 0 || prot < 0 || carbs < 0 || hyd < 0 || adher < 0) {
    		throw new ExMathError();
    	}
        return 0.3*cal + 0.1*fats + 0.15*prot + 0.15*carbs + 0.2*hyd + 0.1*adher;
    }

    public double workout_biased_weight(int sets, int reps, int minutes) throws ExMathError{
    	if (sets<0 || reps < 0 || minutes < 0) {
    		throw new ExMathError();
    	}
    ...
    public double hyd_score(double x, double cap) throws ExMathError{
    ...
    public void conductFoodAnalysis(ArrayList<Food> expected, ArrayList<Food> actual) {
       ...
       catch(ExMathError | ExEmptyInput e) {
    ...
    public void conductWorkoutAnalysis(ArrayList<ArrayList<Workout>> expected, ArrayList<ArrayList<Workout>> actual) {
    	...
    	catch (ExEmptyInput | ExMathError e) {
    		System.out.println(e.getMessage());
    	}
    }
}
